<template>
	<div id="dimmBackground">
		<div id="modal">
			<img style="height: 150px; margin-top: -100px" src="/images/workingKitty.gif" alt="working kitty">

			<ul style="margin-top: 5px">
				<li @click="tempSelectedProject = 'all projects'" :class="{ 'selected': tempSelectedProject === 'all projects' }" class="allProjectsLi">
					all projects
				</li>
				<li v-for="project in projects" :key="project.id" class="projectLi">
					...
					<div v-else-if="deletingProjectId === project.id" class="deleteConfirmContainer">
			...
			<textButton class="textButton" @click="handleSelect" text="Select" variant="lightVersion"/>
		</div>
	</div>
</template>
...
.projectItem, .editContainer, .deleteConfirmContainer {
	display: flex;
...
.actionIcons i, .editIcons i, .deleteIcons i {
	cursor: pointer;
	padding: 4px;
	margin-left: 8px;
	border-radius: 4px;
